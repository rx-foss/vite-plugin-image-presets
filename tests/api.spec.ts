...
describe("inferDimensions", () => {
  ...
  test("generatePlaceholder", async () => {
    const resolved = await getResolvedImage(
      {
        ...formatPreset({
          formats: {
        ...
        generatePlaceholder: async () => "data:image/png;base64,placeholder",
      },
      false
    );
    expect(resolved[0]).toHaveProperty("src", "data:image/png;base64,placeholder");
  });
});
